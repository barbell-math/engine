...
func TestFill(t *testing.T){
    ...
    testUtil.BasicTest(4,len(m1.V),"Rows are not correct.",t);
    testUtil.BasicTest(3,len(m1.V[0]),"Columns are not correct.",t);
...
func TestEquals(t *testing.T){
    ...
    testUtil.BasicTest(false,res,"Equal returned false positive.",t);
    m2=NewMatrix[int](3,4,func(r int, c int) int {
        return c-1;
    });
    res,err=m1.Equals(&m2,0);
    testUtil.BasicTest(true,res,"Equal returned false negative.",t);
    testUtil.BasicTest(nil,err,"Equal returned error it wasn't supposed to.",t);
    m2.AddScalar(1);
    res,err=m1.Equals(&m2,0);
    testUtil.BasicTest(false,res,"Equal returned false positive.",t);
    testUtil.BasicTest(nil,err,"Equal returned error it wasn't supposed to.",t);
...
func TestMulScalar(t *testing.T){
    ...
    m1.MulScalar(2);
    m1.Iter(func(r int, c int, v int){
        testUtil.BasicTest((c+1)*2,v,"Addition was not executed properly.",t);
    });
}
...
func TestInverse(t *testing.T){
    ...
    if !util.IsInverseOfNonSquareMatrix(err) {
        testUtil.FormatError(
            util.InverseOfNonSquareMatrix(""),err,
    ...
    if !util.IsSingularMatrix(err) {
        testUtil.FormatError(
            util.SingularMatrix(""),err,
            "Inverse of singular matrix did not raise error.",t,
        );
    }
    m1=NewMatrix[float32](4,4,IdentityFill[float32]);
    _,err=m1.Inverse();
    testUtil.BasicTest(nil,err,"Inverse returned error it wasn't supposed to.",t);
    ...
    testUtil.BasicTest(true,res,"Inverse was not calculated correctly.",t);
}

func TestRcond(t *testing.T){
    m1:=NewMatrix[float32](4,4,IdentityFill[float32]);
    rcond,err:=m1.Inverse();
    testUtil.BasicTest(float64(1),rcond,"Rcond was not calculated correctly.",t);
    ...
    if !util.IsMatrixSingularToWorkingPrecision(err) {
        testUtil.FormatError(
            util.MatrixSingularToWorkingPrecision(""),err,
