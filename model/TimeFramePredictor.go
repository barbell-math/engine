...
func NewTimeFramePredictor(
        minTimeFrame int,
        maxTimeFrame int,
        window int) (TimeFramePredictor,error) {
    rv:=TimeFramePredictor{
    ...
    if minTimeFrame>maxTimeFrame {
        return rv,util.InvalidPredictionState("min time frame > max time frame");
    }
    return rv,nil;
}
...
//thread.
func (p TimeFramePredictor)UpdateModelStates(
        c *db.CRUD,
        clientID int,
        rv chan<- []error) {
    ...
    cntr:=0;
    errs:=make([]error,0);
    ch:=make(chan StateGenerationRes);
    if err:=formatModelDataError(db.CustomReadQuery(c,p.msMissingQuery(),[]any{
            clientID,
        }, func(n *needsModelState){
            go p.GenerateModelState(c,db.TrainingLog{
                ExerciseID: n.ExerciseID,
                ClientID: clientID,
                DatePerformed: n.Date,
            },ch);
            cntr++;
            fmt.Printf("DB: %+v\n",c.Stats());
            fmt.Printf("%+v\n",n);
...
//parallel. Not having a pointer receiver ensures values are copied to the new
//thread.
func (p TimeFramePredictor)GenerateModelState(
        c *db.CRUD,
        tl db.TrainingLog,
        ch chan<- StateGenerationRes){
...
func (p *TimeFramePredictor)getActualVals(
        c *db.CRUD,
...
func (p *TimeFramePredictor)calcAndSetModelState(date time.Time) error {
    ...
    for _,iter:=range(p.actualVals) {
        if pred,err:=getPredFromLinRegResult(res,&iter); err==nil {
            newMse+=mathUtil.SquareError(iter.Intensity,pred);
        } else {
            return err;
        }
    }
...
func (p *TimeFramePredictor)msStateSelectionQuery(t *time.Time) string {
...
func (p *TimeFramePredictor)msMissingQuery() string {
