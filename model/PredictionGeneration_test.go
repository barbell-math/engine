...
//const TEST_PS_NAME string="TestSG";
//
//func createPredictionData(createMs bool, createTl bool) (func ()){
//    msId,tlId:=[]int{}, []int{};
//    sgId,_:=db.Create(&testDB,db.StateGenerator{T: TEST_SG_NAME});
//    psId,_:=db.Create(&testDB,db.PotentialSurface{T: TEST_PS_NAME});
//    if createMs {
//        msId,_=db.Create(&testDB,db.ModelState{
//            ClientID: 1, ExerciseID: 15, 
//            StateGeneratorID: sgId[0], PotentialSurfaceID: psId[0],
...
func TestGeneratePrediction(t *testing.T){
    ...
    sg,_:=db.GetStateGeneratorByName(&testDB,"Sliding Window");
    ps,_:=db.GetPotentialSurfaceByName(&testDB,"Basic Surface");
    _,err:=GeneratePrediction(&testDB,&tl,
        stateGen.StateGeneratorId(sg.Id),
        potSurf.PotentialSurfaceId(ps.Id),
    );
...
func TestGeneratePredictionNoStateGenerator(t *testing.T){
    ...
    sg:=db.StateGenerator{ T: "Non-existent state generator", Id: 0 };
    ps,_:=db.GetPotentialSurfaceByName(&testDB,"Basic Surface");
    _,err:=GeneratePrediction(&testDB,&tl,
        stateGen.StateGeneratorId(sg.Id),
        potSurf.PotentialSurfaceId(ps.Id),
