...
func CSVFileSplitter(src string, delim byte, hasHeaders bool) iter.Iter[[]string] {
    fileIter:=iter.FileLines(src);
    return func(f iter.IteratorFeedback) ([]string, error, bool) {
        if f==iter.Break {
            _,err,_:=fileIter(iter.Break);
            return []string{},err,false;
        }
        line,err,cont:=fileIter(f);
        if err==nil && cont {
            cols:=splitLineForCSV(line,delim);
            return cols,err,true;
        }
        return []string{},err,false;
    }
}

func splitLineForCSV(row string, delim byte) []string {
    prevIndex:=0;
    inQuotes:=false;
